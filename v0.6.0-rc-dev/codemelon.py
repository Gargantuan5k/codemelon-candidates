...
class Lexer:
    ...
    def make_string(self):
        ...
        while self.current_char is not None and (
                self.current_char != '"' and self.current_char != "'" or escape_character):
            if escape_character:
        ...
        if res.error:
            return res.failure(
                InvalidSyntaxError(
                    ...
                    "Expected 'return', 'continue', 'break', 'var', 'if', 'for', 'while', 'func', int, float, "
                    "identifier, '+', '-', '(', '[' or 'not'"))
...
class RTResult:
    ...
    def register(self, res):
        try:
            ...
            print('Bug code C001, in class RTResult, <class function register(self, res)>')
        return res.value
    ...
    def should_return(self):
        # Note: this will allow you to continue and break outside the current
        # function
        return (
            self.error or
...
class Number(Value):
    ...
    def notted(self):
        return Number(
            1 if self.value == 0 else 0).set_context(
            self.context), None
...
class String(Value):
    ...
    # Functions get_comparison_eq and get_comparison_ne fix a bug where
    def get_comparison_eq(self, other):
        if isinstance(
                other,
                String):  # if statements wouldn't compare STRING datatypes. Fixed (21-01-21)
            return Number(int(self.value == other.value)
                          ).set_context(self.context), None
        else:
            return None, Value.illegal_operation(self, other)
...
class BuiltInFunction(BaseFunction):
    ...
    def execute_pop(self, exec_ctx):
        ...
        if not isinstance(list_, List):
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                'First argument (list) must be a list',
                exec_ctx
            ))
        ...
        if not isinstance(index, Number):
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                'Second argument (index) must be a number',
                exec_ctx
            ))
        ...
        try:
            ...
            return RTResult().failure(
                RTError(
                    self.pos_start,
                    self.pos_end,
                    'Element at index {} could not be popped because index is out of range.'.format(
                        index.value),
                    exec_ctx))
    ...
    def execute_extend(self, exec_ctx):
        ...
        if not isinstance(listA, List):
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                'First argument (listA) must be a list',
                exec_ctx
            ))
        ...
        if not isinstance(listB, List):
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                'Second argument (listB) must be a list',
                exec_ctx
            ))
    ...
    def execute_randint(self, exec_ctx):
        start_value = exec_ctx.symbol_table.get('start_value')
        end_value = exec_ctx.symbol_table.get('end_value')

        if not isinstance(start_value, Number):
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                'First argument (start_value) must be an integer.',
                exec_ctx
            ))
        ...
        if not isinstance(end_value, Number):
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                'Second argument (end_value) must be an integer.',
                exec_ctx
            ))

        return RTResult().success(Number(randint(start_value.value, end_value.value)))

    execute_randint.arg_names = ['start_value', 'end_value']
    ...
    def execute_math_sin(self, exec_ctx):
        ...
        if not isinstance(value, Number):
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                'Argument (value) must be an integer',
                exec_ctx
            ))
    ...
    def execute_math_rem(self, exec_ctx):
        ...
        if not isinstance(value1, Number):
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                'Argument 1 (value1) must be a number',
                exec_ctx
            ))
        ...
        if not isinstance(value2, Number):
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                'Argument 2 (value2) must be a number',
                exec_ctx
            ))
        return RTResult().success(Number(value1.value % value2.value))

    execute_math_floor.arg_names = ['value1', 'value2']

    def execute_math_floordiv(self, exec_ctx):
        value1 = exec_ctx.symbol_table.get('value1')
        value2 = exec_ctx.symbol_table.get('value2')
    ...
    execute_math_floordiv.arg_names = ['value1', 'value2']
    ...
    def execute_run(self, exec_ctx):
        ...
        if not isinstance(filename, String):
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                'Argument (filename) must be a string',
                exec_ctx
            ))
        elif not filename.value.endswith('.mln'):
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                'Could not run file: File Type not supported',
                exec_ctx
            ))
        ...
        if error:
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                f"Failed to execute script \"{filename}\"\n" +
                error.as_string(),
                exec_ctx
            ))
...
BuiltInFunction.math_floordiv = BuiltInFunction('math_floordiv')
BuiltInFunction.math_rem = BuiltInFunction('math_rem')
...
class Interpreter:
    ...
    def visit_ForNode(self, node, context):
        ...
        return res.success(Number.null if node.should_return_null else List(
...
global_symbol_table.set('math_floordiv', BuiltInFunction.math_floordiv)
global_symbol_table.set('math_rem', BuiltInFunction.math_rem)
